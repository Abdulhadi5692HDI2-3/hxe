...
static void emitLoop(int loopStart) {
    ...
    if (offset > UINT16_MAX) error("why tf your loop so large? (offset > 65535)");
...
static void patchJump(int offset) {
    ...
    if (jump > UINT16_MAX) {
        error("Too much code to jump over! (jump > 65535)");
    }
...
static void tenary(bool canAssign);
...
static void super_(bool canAssign) {
    if (currentClass == NULL) {
        ...
        error("Can't use 'super' in a class without any super classes!");
    }
...
static void this_(bool canAssign) {
    if (currentClass == NULL) {
        error("Can't use 'this' out of a class!");
        return;
    }
    variable(false);
}
...
static void addLocal(Token name) {
    if (current->localCount == UINT8_COUNT) {
        error("Too many variables in one function/block!");
        return;
    }
    Local* local = &current->locals[current->localCount++];
    local->name = name;
...
static void declareVariable() {
    ...
    for (int i = current->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            error("Already a variable in this function/block/scope");
        }
    }
    addLocal(*name);
}
...
static void binary(bool canAssign) {
    ...
    switch(operatorType) {
        ...
        case TOKEN_PERCENTAGE: emitByte(OP_MODULO); break;
        default: return;
    }
}
...
static void expressionStatement() {
    expression();
    consume(TOKEN_SEMICOLON, "Expect ';' to end expression!");
    emitByte(OP_POP);
}
...
static void printStatement() {
    expression();
    consume(TOKEN_SEMICOLON, "Expect ';' to end instruction.");
    emitByte(OP_PRINT);
}

static void returnStatement() {
    if (current->type == TYPE_SCRIPT) {
        error("Can't return from top-level. (not in a function)!");
    }
    if (match(TOKEN_SEMICOLON)) {
        emitReturn();
    } else {
        if (current->type == TYPE_INITALIZER) {
            error("Can't return a value from an initalzier!!!!!");
        }
        expression();
